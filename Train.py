...
root = '/Users/rudrathaker/Desktop/Neural Networks/Project'
 
def dice_(gt_mask, pred, smooth = 1e-5):
    
    gt_mask_flat = tf.keras.backend.flatten(gt_mask)
    pred_flat = tf.keras.backend.flatten(pred)
    intersect = tf.keras.backend.sum(gt_mask_flat * pred_flat, axis = -1)
    denom = tf.keras.backend.sum(gt_mask_flat + pred_flat, axis = -1)
    coef = tf.keras.backend.mean(((2 * intersect + smooth) / (denom + smooth) ))
...
def generate_graphs(train_loss, val_loss, val_dice, val_iou):
    ...
    plt.savefig('/Users/rudrathaker/Desktop/Neural Networks/Project/Graphs/' + datetime.now().strftime("%H:%M:%S") + '.jpg')

@tf.autograph.experimental.do_not_convert
def train(model, epochs, base_lr):
    
    save_dir = '/Users/rudrathaker/Desktop/Neural Networks/Project/Model/'
